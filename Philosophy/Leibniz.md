...
- All cockerspaniels are dogs.
	...
	+ Symbolically, cockerspaniels can be represented as {\\( \mathrm{dog}, q_1, q_2, … , q_n \\)} where qs are qualities unique to cockerspaniels and "dog" is the set of all concepts in dog.
	...
	+ Because {dog} \\( \in \\) {\\( \mathrm{dog}, q_1, q_2, … , q_n \\)}, all cocker spaniels are dogs.
- All triangles have three angles.
	+ Triangle's are complex ideas experienced through: {shape, polygon, three angles, three edges, …}
	+ {three angles} \\(\in\\) {shape, polygon, three angles, three edges, …}
	+ So \\( \forall x \\) if x is a triangle \\( \to \\) x has three angles
- Water is liquid at room temperature
...
- If a proposition p is false, then the contradictory of p is true. \\( p \\) and \\(\lnot p \\) are contradictory.
...
- 'Discourse on Metaphysics' part 9
	...
	2. Sunstance begins only by creation and cease to exist only by annihilation
